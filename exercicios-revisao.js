...
const clothes2 = { shoes: { colors: ['yellow', 'purple'] }, ...clothes };
...
let raizQuadrada = numbers.map((item) => {
...
let multiplica10 = numbers.map((item) => item * 10);
...
let maiores18 = ages.filter((item) => item >= 18);
...
let dataTypeDog = data.filter((item) => item.type == 'dog');
//Por fim, ainda utilizando o array acima, retorne a soma da idade de todos os cachorros (​type: dog)​
let somaDog = dataTypeDog.reduce((acumulador, item) => {
  return acumulador + item.age;
}, 0);
...
const nomeTodo = persons.map((item) => `${item.firstname} ${item.lastname}`);
